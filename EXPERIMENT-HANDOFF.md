...
| Stage D: typed model checker (2026-09-17) | `claims/modelcheck.py`, `shen/modelcheck/`, `capcov experiment claims modelcheck --model DIR --out DIR`, `tests/claim_semantics/test_modelcheck.py` | Shen's type checker decides well-formedness of an immutable copied model snapshot. Model code may generate only canonical literal units; each is validated and typechecked in an isolated model-free process. Certificates and synthesized transcripts bind source/runtime digests without local paths. `model_well_formed` requires runtime reproduction, and reviewer admission is a separate caller input pinned to the exact model/checker/version/certificate; receipt-local self-admission is ignored. |
