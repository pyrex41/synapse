"""
CLI for /sync-docs evaluations.

Usage:
    # Run single test case
    sync-docs-eval run case_001_tdd_api_change --tag baseline

    # Run all test cases
    sync-docs-eval run-all --tag baseline

    # Compare runs
    sync-docs-eval compare baseline experiment

    # List available test cases
    sync-docs-eval list-cases

    # Show run history
    sync-docs-eval history
"""
...
from .runner import SyncDocsRunner, run_sync_docs
from discover_evals.report import generate_report, load_runs
...
def cmd_run(case_id: str, tag: str = None, mock: bool = False, api: bool = False):
    ...
    console.print(f"  Docs Searched: {result.docs_searched}")
    console.print(f"  Docs Updated: {len(result.updated_docs)}")
...
def cmd_list_cases():
    ...
    table = Table(title="Available Sync-Docs Test Cases")
...
def cmd_history():
    ...
    table = Table(title="Sync-Docs Run History")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="/sync-docs evaluation framework",
    ...
    run_parser.add_argument("case_id", help="Test case ID (e.g., case_001_tdd_api_change)")
