...
var require_dist = __commonJS({
  "../../node_modules/ajv-formats/dist/index.js"(exports, module) {
    ...
    var formatsPlugin = (ajv, opts = { keywords: true }) => {
      ...
      addFormats2(ajv, list3, formats, exportName);
      if (opts.keywords)
        limit_1.default(ajv);
      return ajv;
    };
    ...
    function addFormats2(ajv, list3, fs, exportName) {
      var _a;
      var _b;
    ...
    if (!valid && validate.errors) {
      for (const error of validate.errors) {
        ...
        issues.push({
          ...
          message: `${path2} ${message}`,
          field: path2.replace(/^\//, "") || void 0
        });
      }
    }
  } catch (error) {
...
function extractHeadingText(heading2) {
  let text5 = "";
  for (const child of heading2.children) {
    if (child.type === "text") {
      text5 += child.value;
    } else if (child.type === "html") {
      continue;
    } else if ("children" in child) {
      text5 += extractInlineText(child.children);
    }
  }
  return text5.trim();
}
...
function findSections(ast) {
  ...
  for (const node2 of ast.children) {
    if (node2.type === "heading" && node2.depth === 2) {
      ...
      const title = extractHeadingText(node2);
      currentSection = {
        id: normalizeSectionId(title),
        title,
        depth: node2.depth,
        line: node2.position?.start.line || 0,
        column: node2.position?.start.column || 0,
        content: []
      };
    } else if (currentSection) {
      currentSection.content.push(node2);
    }
  }
  if (currentSection) {
    sections.push(currentSection);
...
function validateBodyGrammar(body, grammar, docType) {
  const issues = [];
  ...
  for (const rule of grammar.sections) {
    const found = sections.find(
      (s) => s.id === rule.id || s.title.toLowerCase() === rule.title.toLowerCase()
    );
...
## ${rule.title}`,
  ...
  const ruleOrder = new Map(grammar.sections.map((r) => [r.id, r.order]));
  const ruleById = new Map(grammar.sections.map((r) => [r.id, r]));
  let lastOrder = 0;
  let lastSectionTitle = "";
  for (const section of sections) {
    const order2 = ruleOrder.get(section.id);
    if (order2 !== void 0 && order2 < lastOrder) {
      const expectedBefore = grammar.sections.filter((r) => r.order < order2).sort((a, b) => b.order - a.order)[0];
      issues.push({
        type: "error",
...
function validateSectionShape(section, rule) {
  const issues = [];
  const shape = rule.shape;
  if (shape.type === "list") {
    const lists = section.content.filter((n) => n.type === "list");
    const listTypeExample = shape.ordered ? "1. First item\n2. Second item\n3. Third item" : "- First item\n- Second item\n- Third item";
...
${listTypeExample}`,
    ...
    for (const list3 of lists) {
      if (shape.ordered !== void 0 && list3.ordered !== shape.ordered) {
        const currentType = list3.ordered ? "numbered (1. 2. 3.)" : "bulleted (-)";
...
${listTypeExample}`,
      ...
      if (shape.minItems && list3.children.length < shape.minItems) {
        issues.push({
          type: "error",
          code: "INSUFFICIENT_LIST_ITEMS",
          message: `Section "## ${rule.title}" needs at least ${shape.minItems} list items (currently has ${list3.children.length}).`,
          sectionId: rule.id
        });
      }
    }
...
| Data     | Data     |`,
  ...
  if (shape.type === "flow" && shape.allowedNodes) {
    ...
    for (const node2 of section.content) {
      if (!shape.allowedNodes.includes(node2.type)) {
        const nodeName = nodeTypeNames[node2.type] || node2.type;
        const allowedNames = shape.allowedNodes.map((t) => nodeTypeNames[t] || t).join(", ");
        issues.push({
          type: "error",
...
function extractWikilinks(content3) {
  const wikilinks = [];
  const regex = /\[\[([^\]|]+)(?:\|[^\]]+)?\]\]/g;
  let match;
  while ((match = regex.exec(content3)) !== null) {
...
function validateWikilinks(body, vaultIndex) {
  const issues = [];
  const wikilinks = extractWikilinks(body);
  const normalizedIndex = new Set(
    vaultIndex.map((f) => f.toLowerCase().replace(/\s+/g, "-"))
  );
  for (const link2 of wikilinks) {
    const normalized = link2.toLowerCase().replace(/\s+/g, "-");
    const exists = normalizedIndex.has(normalized) || normalizedIndex.has(normalized.replace(/\.md$/, "")) || [...normalizedIndex].some(
      (f) => f.endsWith(`/${normalized}`) || f.endsWith(`-${normalized}`) || f === normalized
    );
    if (!exists) {
      issues.push({
        type: "error",
        code: "BROKEN_LINK",
        message: `Broken link: [[${link2}]] - This document doesn't exist. Check the spelling or create the document first.`
      });
    }
  }
  return issues;
}
function validateCrossLinks(frontmatter, body, vaultIndex) {
  const issues = [];
  ...
  switch (docType) {
    case "process":
      if (!frontmatter.related_standards || !Array.isArray(frontmatter.related_standards) || frontmatter.related_standards.length === 0) {
        issues.push({
          type: "error",
      ...
      break;
    case "sop":
      if (!frontmatter.related_process || typeof frontmatter.related_process !== "string" || frontmatter.related_process.trim() === "") {
        issues.push({
          type: "error",
          code: "MISSING_REQUIRED_LINK",
...
function validateDocument(frontmatter, body, filePath, context) {
  const issues = [];
  const docType = frontmatter.type;
  if (!docType) {
    issues.push({
